...
export class CliHandler {
  ...
  async start(): Promise<void> {
    ...
    console.log('='.repeat(50));
    console.log('\nCommands:');
    console.log('  auth <email>  — Authenticate via Google OAuth');
  ...
  private async handleAuth(email?: string): Promise<void> {
    if (!email) {
      console.log('\n⚠️  Usage: auth <email@gmail.com>\n');
      return;
    }

    const user = await this.authService.authenticate(email);
    console.log(`✅ Authenticated as: ${user.profile.name ?? user.email}\n`);
  }
